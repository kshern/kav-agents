...
export class YFinanceUtils {
  ...
  async getIncomeStmt(symbol: string) {
    return await yahooFinance.quoteSummary(symbol, { modules: ['incomeStatementHistory'] });
  }
  ...
  async getBalanceSheet(symbol: string) {
    return await yahooFinance.quoteSummary(symbol, { modules: ['balanceSheetHistory'] });
  }
  ...
  async getCashFlow(symbol: string) {
    return await yahooFinance.quoteSummary(symbol, { modules: ['cashflowStatementHistory'] });
  }
  ...
  async getAnalystRecommendations(symbol: string) {
    return await yahooFinance.quoteSummary(symbol, { modules: ['recommendationTrend'] });
  }
}
